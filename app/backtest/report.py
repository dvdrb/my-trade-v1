...
def write_report(run_id: str, summary: dict[str, Any], trades: list[Trade], open_trades: list[Trade], signals: list[Signal], equity_curve: list[tuple[int, float]]) -> Path:
    ...
    _write_trades(directory / "trades.csv", trades)
    _write_trades(directory / "open_trades.csv", open_trades)
    ...
    with (directory / "signals.csv").open("w", newline="", encoding="utf-8") as file:
        ...
        for signal in signals:
            ...
            row["metadata"] = json.dumps(signal.metadata)
            writer.writerow(row)
