...
def test_compare_backtests_reads_two_summary_files(tmp_path) -> None:
    ...
    strict.write_text(json.dumps({"strategy_version": "strict", "config_name": "strategy_nested_mtf_hard_1h", "symbol": "BTC", "closed_trades": 2, "accepted_signals": 5, "win_rate": 0.5, "expectancy_r": 0.2, "profit_factor": 1.3, "max_drawdown": 0.01, "max_losing_streak": 1, "nested_candidate_funnel": {"rejected_by_mtf_zone": 7}}))
    ...
    assert "Hard 1h only" in output and "BTC" in output and "rejected_by_mtf_zone" in output
